...
import {
  ...
  Maximize2, Download, Sun, Pencil, Check,
} from 'lucide-react';
...
async function compressImageForUpload(file: File, maxDimension = 1568, quality = 0.85): Promise<File> {
...
function YardMapView({
  ...
  return (
    <div>
      <div className="relative w-full aspect-[4/3] rounded-2xl border border-gray-200 shadow-inner" onClick={closeAll}>
        ...
        {existingPlantsMap.map((p, i) => {
          ...
          return (
            <div key={key}>
              <button
                type="button"
                className="absolute -translate-x-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-stone-400 border-2 border-white shadow-md flex items-center justify-center overflow-hidden hover:ring-2 hover:ring-forest-400 transition-shadow"
        ...
        {recommendations.map((r, i) => {
          ...
          return (
            <div key={key}>
              <button
                type="button"
                className="absolute -translate-x-1/2 -translate-y-1/2 w-9 h-9 rounded-full border-2 border-white shadow-md overflow-hidden bg-forest-100 hover:ring-2 hover:ring-forest-400 transition-shadow"
      ...
      <p className="text-[11px] text-gray-400 mt-1">
        ...
        Hover a marker for details, click to keep it open, and use "Correct ID" if a plant was misidentified.
      </p>
    </div>
  );
}
...
function ResultBody({
  ...
  onRetry: () => void;
  onStructuredChange: (updater: (prev: StructuredAnalysis) => StructuredAnalysis) => void;
}) {
  if (structured) {
    return <StructuredAnalysisView data={structured} onChange={onStructuredChange} />;
  }
...
export default function VisualizePage() {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    compressImageForUpload(file)
      .then(applyFile)
      .catch(() => applyFile(file)); // fall back to the original file if compression fails
  }, []);
        ...
        <div className="card p-6 animate-fade-in">
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            <ResultBody
              ...
              onRetry={() => runAnalysis()}
              onStructuredChange={updater => setStructured(prev => (prev ? updater(prev) : prev))}
            />
          </div>
